...
**Testing Code Mapping:** If a commit contains changes **only** in tests, testing projects, and/or test-automation tooling (and no other source code),
the commit type **MUST** be `test`.
...
- **Component Prefixing:** When a commit affects a specific component, class, or method, you may prefix the description with the
  EXACT name of that entity (preserving its original capitalization).
